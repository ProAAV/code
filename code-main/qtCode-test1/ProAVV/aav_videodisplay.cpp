    ...
    ui(new Ui::VideoDisplay)
{
   ...
   // m_btn_rate=new QPushButton(this);
    m_btn_rate=new Videoratebutton(this);
    m_btn_rate->setText("Rate");
    ...
    //播放视频
    m_player->setMedia(QUrl::fromLocalFile("/home/Pluto/Desktop/1158817917-1-192.mp4"));
    ...
    //实现倍速同步
    connect(m_btn_rate->getSlider(), &QSlider::sliderMoved, this, [this](int value) {
            double speed = value / 100.0; // 计算倍速
            this->m_player->setPlaybackRate(speed); // 设置播放速度
            m_btn_rate->m_lab_rate->setText(QString("%1x").arg(speed));
    });
}
...
bool VideoDisplay::eventFilter(QObject *obj,QEvent *event)
{
    if(obj==m_btn_rate)
    {
        if(event->type()==QEvent::Enter){
            m_slider_video_rate->show();
            return true;
        }
        else if(event->type()==QEvent::Leave)
        {
            m_slider_video_rate->hide();
            return true;
        }
    }
